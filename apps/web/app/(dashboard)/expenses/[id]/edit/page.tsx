    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50/50">
      ...
      <div className="p-4 border-b border-border bg-card flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-semibold text-lg">Edit Expense</h1>
        </div>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-2 lg:col-span-2">
                  ...
                  {mastersLoaded ? (
                    <Controller
                      ...
                      render={({ field }) => (
                        <Select 
                          ...
                          onValueChange={(val) => {
                            if (val !== "none") field.onChange(val);
                          }}
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="none" className="hidden" disabled>Select Supplier</SelectItem>
                            {suppliers.map(s => (
                    ...
                    <div className="h-10 w-full rounded-md border border-slate-200 bg-slate-50 flex items-center px-3 text-slate-500 text-sm animate-pulse">
                      Loading...
                    </div>
                  )}
                </div>
      ...
      <SettleExpenseDialog 
        ...
        onClose={async () => {
          ...
          if (autoSettle && loadedExpense) {
            try {
              const res = await api.get(`/expenses/${loadedExpense.id}/settlements`);
              if (!res.data || res.data.length === 0) {
                // User cancelled settlement, revert status to Unpaid
